	...
	public static class ConnectionExt
	{
			...
			where T : class
		{
			...
			using (var command = connection.CreateCommand())
			{
				...
				{
					var id = CommandRunner.ExecuteCommandAndLog(command, c => c.ExecuteScalar());
					map.Identifiers[0].SetValue(t, id);
					return t;
				}
				CommandRunner.ExecuteCommandAndLog(command, c => c.ExecuteNonQuery());
			}
		...
		private static int AttachParamsAndRun(IDbConnection connection, string sql, IList<object> parameters)
		{
			using (var command = connection.CreateCommand())
			{
					...
					return CommandRunner.ExecuteCommandAndLog(command, c => c.ExecuteNonQuery());
			}
